...
// Admin service provides administrative commands and debugging tools.
// This service provides utility functions for development and debugging.
// It is kept separate from core business logic as it's not part of the game domain.
...
type AdminService interface {
	...
	OnAdminSetResources(ctx context.Context, gameID, playerID string, resources playerPkg.Resources) error

	// OnAdminSetProduction sets a player's production
	OnAdminSetProduction(ctx context.Context, gameID, playerID string, production playerPkg.Production) error
...
type AdminServiceImpl struct {
	...
	cardDeckRepo   cardPkg.CardDeckRepository
...
func (s *AdminServiceImpl) OnAdminGiveCard(ctx context.Context, gameID, playerID, cardID string) error {
	...
	// Living Card Instance Pattern: Pass Card instance, not ID
	if err := s.playerRepo.AddCard(ctx, gameID, playerID, *card); err != nil {
...
// OnAdminSetResources sets a player's resources
func (s *AdminServiceImpl) OnAdminSetResources(ctx context.Context, gameID, playerID string, resources playerPkg.Resources) error {
	log := logger.WithGameContext(gameID, playerID)
...
// OnAdminSetProduction sets a player's production
func (s *AdminServiceImpl) OnAdminSetProduction(ctx context.Context, gameID, playerID string, production playerPkg.Production) error {
	log := logger.WithGameContext(gameID, playerID)
...
func (s *AdminServiceImpl) OnAdminSetGlobalParameters(ctx context.Context, gameID string, params parameters.GlobalParameters) error {
	...
	// Get session to access parameters service
	gameSession, err := s.sessionRepo.GetByID(ctx, gameID)
	if err != nil {
		log.Error("Failed to get session", zap.Error(err))
		return fmt.Errorf("failed to get session: %w", err)
	}

	// Set parameters via the service (which uses Set methods that publish events)
	if err := gameSession.ParametersService.SetTemperature(ctx, params.Temperature); err != nil {
		log.Error("Failed to set temperature", zap.Error(err))
		return fmt.Errorf("failed to set temperature: %w", err)
	}

	if err := gameSession.ParametersService.SetOxygen(ctx, params.Oxygen); err != nil {
		log.Error("Failed to set oxygen", zap.Error(err))
		return fmt.Errorf("failed to set oxygen: %w", err)
	}

	if err := gameSession.ParametersService.SetOceans(ctx, params.Oceans); err != nil {
	...
	// Broadcast updated game state
	if err := s.sessionManager.Broadcast(gameID); err != nil {
		log.Error("Failed to broadcast game state after parameters change", zap.Error(err))
		// Don't fail the operation, just log the error
	}
...
func (s *AdminServiceImpl) OnAdminStartTileSelection(ctx context.Context, gameID, playerID, tileType string) error {
	...
	// Get session to access the board service
	gameSession, err := s.sessionRepo.GetByID(ctx, gameID)
	if err != nil {
		log.Error("Failed to get session", zap.Error(err))
		return fmt.Errorf("failed to get session: %w", err)
	...
	availableHexes, err := s.calculateDemoAvailableHexes(gameSession, tileType)
	if err != nil {
	...
	pendingSelection := &playerPkg.PendingTileSelection{
		TileType:       tileType,
...
func (s *AdminServiceImpl) OnAdminSetCurrentTurn(ctx context.Context, gameID, playerID string) error {
	...
	// Get session to access turn order service
	gameSession, err := s.sessionRepo.GetByID(ctx, gameID)
	if err != nil {
		log.Error("Failed to get session", zap.Error(err))
		return fmt.Errorf("failed to get session: %w", err)
	...
	// Set current turn via service
	if err := gameSession.TurnOrderService.SetCurrentTurn(ctx, &playerID); err != nil {
...
func (s *AdminServiceImpl) calculateDemoAvailableHexes(gameSession *sessionPkg.Session, tileType string) ([]string, error) {
	var availableHexes []string

	// Get board from session
	board, err := gameSession.BoardService.GetBoard(context.Background())
	if err != nil {
...
func (s *AdminServiceImpl) OnAdminSetCorporation(ctx context.Context, gameID, playerID, corporationID string) error {
	...
	if corporationCard.StartingResources != nil {
		resources := playerPkg.Resources{
			Credits:  (*corporationCard.StartingResources)[domain.ResourceCredits],
			Steel:    (*corporationCard.StartingResources)[domain.ResourceSteel],
			Titanium: (*corporationCard.StartingResources)[domain.ResourceTitanium],
			Plants:   (*corporationCard.StartingResources)[domain.ResourcePlants],
			Energy:   (*corporationCard.StartingResources)[domain.ResourceEnergy],
			Heat:     (*corporationCard.StartingResources)[domain.ResourceHeat],
		}
	...
	if corporationCard.StartingProduction != nil {
		production := playerPkg.Production{
			Credits:  (*corporationCard.StartingProduction)[domain.ResourceCredits],
			Steel:    (*corporationCard.StartingProduction)[domain.ResourceSteel],
			Titanium: (*corporationCard.StartingProduction)[domain.ResourceTitanium],
			Plants:   (*corporationCard.StartingProduction)[domain.ResourcePlants],
			Energy:   (*corporationCard.StartingProduction)[domain.ResourceEnergy],
			Heat:     (*corporationCard.StartingProduction)[domain.ResourceHeat],
		}
	...
	paymentSubstitutes := []playerPkg.PaymentSubstitute{}
	for _, behavior := range corporationCard.Behaviors {
		...
		for _, trigger := range behavior.Triggers {
			if trigger.Type == cardPkg.ResourceTriggerAuto {
				hasAutoTrigger = true
		...
		for _, output := range behavior.Outputs {
			if output.Type == "payment-substitute" {
				...
				if len(output.AffectedResources) > 0 {
					...
					substitute := playerPkg.PaymentSubstitute{
	...
	// Extract and store corporation passive effects in Player.Effects
	playerEffects := extractPlayerEffects(corporationCard.ID, corporationCard, log)
	if len(playerEffects) > 0 {
		...
		if err := s.playerRepo.UpdatePlayerEffects(ctx, gameID, playerID, updatedEffects); err != nil {
	...
	var manualActions []cardPkg.PlayerAction
	for behaviorIndex, behavior := range corporationCard.Behaviors {
		...
		for _, trigger := range behavior.Triggers {
			if trigger.Type == cardPkg.ResourceTriggerManual {
	...
	if len(manualActions) > 0 {
		...
		newActions := make([]cardPkg.PlayerAction, len(currentPlayer.Actions)+len(manualActions))
	...
	if forcedAction != nil {
		...
		// TODO: Re-implement forced action triggering after ForcedActionManager is restructured
		// For now, just log that the forced action was set
		log.Info("⚠️  Forced first action set but not triggered (needs ForcedActionManager)",
			zap.String("action_type", forcedAction.ActionType))
	}
...
func extractPlayerEffects(cardID string, c *cardPkg.Card, log *zap.Logger) []cardPkg.PlayerEffect {
	var playerEffects []cardPkg.PlayerEffect
	...
	for i, behavior := range c.Behaviors {
		...
		playerEffects = append(playerEffects, cardPkg.PlayerEffect{
			CardID:        cardID,
			CardName:      c.Name,
			BehaviorIndex: i,
			Behavior:      behavior,
