...
type CardDeckRepository interface {
	...
	InitializeDeck(ctx context.Context, gameID string, cards []model.Card) error
...
// InitializeDeck creates a new shuffled deck for a game
func (r *CardDeckRepositoryImpl) InitializeDeck(ctx context.Context, gameID string, cards []model.Card) error {
	r.mutex.Lock()
