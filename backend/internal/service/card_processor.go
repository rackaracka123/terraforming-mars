...
func NewCardProcessor(gameRepo game.Repository, playerRepo player.Repository, cardDeckRepo cardpkg.CardDeckRepository) *CardProcessor {
...
func (cp *CardProcessor) ApplyCardEffects(ctx context.Context, gameID, playerID string, c *cardpkg.Card, choiceIndex *int, cardStorageTarget *string) error {
	log := logger.WithGameContext(gameID, playerID)
	log.Debug("🎭 Applying card effects", zap.String("card_name", c.Name))

	// Apply production effects if the card has them
	if err := cp.applyProductionEffects(ctx, gameID, playerID, c, choiceIndex); err != nil {
...
// choiceIndex is optional and used when the card has choices between different effects
func (cp *CardProcessor) applyProductionEffects(ctx context.Context, gameID, playerID string, c *cardpkg.Card, choiceIndex *int) error {
	log := logger.WithGameContext(gameID, playerID)

	// Get current player to read current production
	p, err := cp.playerRepo.GetByID(ctx, gameID, playerID)
	if err != nil {
		return fmt.Errorf("failed to get player for production update: %w", err)
	}

	// Calculate new production values from card behaviors
	newProduction, err := p.GetProduction()
	if err != nil {
	...
	for _, behavior := range c.Behaviors {
		...
		if len(behavior.Triggers) > 0 && behavior.Triggers[0].Type == "auto" && behavior.Triggers[0].Condition == nil {
...
func (cp *CardProcessor) extractAndAddManualActions(ctx context.Context, gameID, playerID string, c *cardpkg.Card, choiceIndex *int) error {
	log := logger.WithGameContext(gameID, playerID)
	...
	if len(manualActions) > 0 {
		...
		newActions := make([]cardpkg.PlayerAction, len(player.Actions)+len(manualActions))
...
func (cp *CardProcessor) applyVictoryPointConditions(ctx context.Context, gameID, playerID string, c *cardpkg.Card) error {
	log := logger.WithGameContext(gameID, playerID)
...
// choiceIndex is optional and used when the card has choices between different effects
// cardStorageTarget is optional and used when outputs target "any-card" storage
func (cp *CardProcessor) applyResourceEffects(ctx context.Context, gameID, playerID string, c *cardpkg.Card, choiceIndex *int, cardStorageTarget *string) error {
	log := logger.WithGameContext(gameID, playerID)
	...
	for _, behavior := range c.Behaviors {
		...
		if len(behavior.Triggers) > 0 && behavior.Triggers[0].Type == "auto" && behavior.Triggers[0].Condition == nil {
			...
			for _, output := range allOutputs {
				...
				case "animals", "microbes", "floaters", "science", "asteroid":
					// Handle card storage resources
					if err := cp.ApplyCardStorageResource(ctx, gameID, playerID, c.ID, output, cardStorageTarget, log); err != nil {
...
func (cp *CardProcessor) applyTileEffects(ctx context.Context, gameID, playerID string, c *cardpkg.Card) error {
	...
	return cp.playerRepo.CreateTileQueue(ctx, gameID, playerID, c.ID, tilePlacementQueue)
}

// applyCardDrawPeekEffects handles card draw/peek/take/buy effects from card behaviors
func (cp *CardProcessor) applyCardDrawPeekEffects(ctx context.Context, gameID, playerID string, c *cardpkg.Card) error {
	log := logger.WithGameContext(gameID, playerID)
...
func (cp *CardProcessor) ApplyCardStorageResource(ctx context.Context, gameID, playerID, playedCardID string, output cardpkg.ResourceCondition, cardStorageTarget *string, log *zap.Logger) error {
...
// choiceIndex is optional and used when the card has choices between different effects
func (cp *CardProcessor) applyGlobalParameterEffects(ctx context.Context, gameID, playerID string, c *cardpkg.Card, choiceIndex *int) error {
	log := logger.WithGameContext(gameID, playerID)
	...
	if temperatureChange != 0 {
		oldTemperature := globalParams.Temperature
		newTemperature := oldTemperature + temperatureChange
		// Clamp to valid range
		if newTemperature > parameters.MaxTemperature {
			newTemperature = parameters.MaxTemperature
		}
		if newTemperature < parameters.MinTemperature {
			newTemperature = parameters.MinTemperature
		}
	...
	if oxygenChange != 0 {
		oldOxygen := globalParams.Oxygen
		newOxygen := oldOxygen + oxygenChange
		// Clamp to valid range
		if newOxygen > parameters.MaxOxygen {
			newOxygen = parameters.MaxOxygen
		}
		if newOxygen < parameters.MinOxygen {
			newOxygen = parameters.MinOxygen
		}
	...
	if oceansChange != 0 {
		oldOceans := globalParams.Oceans
		newOceans := oldOceans + oceansChange
		// Clamp to valid range
		if newOceans > parameters.MaxOceans {
			newOceans = parameters.MaxOceans
		}
		if newOceans < parameters.MinOceans {
			newOceans = parameters.MinOceans
		}
