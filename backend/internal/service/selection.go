...
func (s *SelectionManager) SelectStartingCards(ctx context.Context, gameID, playerID string, cardIDs []string, corporationID string) error {
	...
	if playerResources.Credits < cost {
		return fmt.Errorf("insufficient credits: need %d, have %d", cost, playerResources.Credits)
	}
...
func (s *SelectionManager) ValidateProductionCardSelection(ctx context.Context, gameID, playerID string, cardIDs []string) error {
	...
	if production.ProductionPhaseState == nil {
	...
	if len(production.ProductionPhaseState.AvailableCards) == 0 {
	...
	playerOptions := production.ProductionPhaseState.AvailableCards
...
func (s *SelectionManager) IsAllPlayersCardSelectionComplete(ctx context.Context, gameID string) bool {
	...
	case game.GamePhaseStartingCardSelection:
...
func (s *SelectionManager) checkStartingCardSelectionComplete(players []player.Player, log *zap.Logger) bool {
	playersCompleted := 0
...
func (s *SelectionManager) checkProductionCardSelectionComplete(players []player.Player) bool {
	...
	for _, player := range players {
		if production.ProductionPhaseState != nil {
			...
			if !production.ProductionPhaseState.SelectionComplete {
...
func (s *SelectionManager) extractForcedFirstAction(corporation *card.Card) *player.ForcedFirstAction {
...
func (s *SelectionManager) determineActionType(outputs []card.ResourceCondition) string {
	if len(outputs) == 0 {
		return "unknown"
	}
...
func (s *SelectionManager) generateForcedActionDescription(outputs []card.ResourceCondition) string {
	if len(outputs) == 0 {
...
func (s *SelectionManager) applyCorporationSelection(ctx context.Context, gameID, playerID, corporationID string, player *player.Player) error {
	log := logger.WithGameContext(gameID, playerID)
	...
	if len(manualActions) > 0 {
		...
		newActions := make([]player.PlayerAction, len(currentPlayer.Actions)+len(manualActions))
