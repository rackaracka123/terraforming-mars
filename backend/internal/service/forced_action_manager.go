...
type ForcedActionManager interface {
	...
	TriggerForcedFirstAction(ctx context.Context, gameID, playerID string, player player.Player) error
}
...
func (m *ForcedActionManagerImpl) SubscribeToPhaseChanges() {
	events.Subscribe(m.eventBus, func(event game.GamePhaseChangedEvent) {
...
func (m *ForcedActionManagerImpl) onPhaseChanged(ctx context.Context, event game.GamePhaseChangedEvent) error {
	log := logger.WithGameContext(event.GameID, "")

	// Only trigger forced actions when transitioning to Action phase
	if event.NewPhase != string(game.GamePhaseAction) {
		return nil
	}
...
func (m *ForcedActionManagerImpl) TriggerForcedFirstAction(ctx context.Context, gameID, playerID string, player player.Player) error {
	...
	var forcedBehavior *card.CardBehavior
	for _, behavior := range corporation.Behaviors {
		for _, trigger := range behavior.Triggers {
			if trigger.Type == card.ResourceTriggerAutoFirstAction {
...
func (m *ForcedActionManagerImpl) triggerCardDrawAction(ctx context.Context, gameID, playerID string, pl player.Player, amount int) error {
	...
	pendingCardDrawSelection := &player.PendingCardDrawSelection{
		...
		CardBuyCost:    0,
		Source:         pl.ForcedFirstAction.CorporationID,
	}
...
func (m *ForcedActionManagerImpl) triggerTilePlacementAction(ctx context.Context, gameID, playerID string, pl player.Player, tileType types.ResourceType) error {
	log := logger.WithGameContext(gameID, playerID)
	...
	pendingTileSelection := &player.PendingTileSelection{
...
func (m *ForcedActionManagerImpl) getAvailableHexesForTileType(g game.Game, tileType types.ResourceType, playerID string) []string {
	var availableHexes []string

	board, err := g.GetBoard()
	if err != nil {
		return availableHexes // Return empty if board unavailable
	}
	...
	for _, tile := range board.Tiles {
		...
		if tileType == types.ResourceGreeneryPlacement {
			if m.isAdjacentToPlayerTile(g, tile.Coordinates, playerID) {
				availableHexes = append(availableHexes, tile.Coordinates.String())
			}
			continue
		}
...
func (m *ForcedActionManagerImpl) isAdjacentToPlayerTile(g game.Game, coord types.HexPosition, playerID string) bool {
	// For first action, if player has no tiles yet, any empty hex is valid
	hasPlayerTile := false
	board, err := g.GetBoard()
	if err != nil {
		return false // Return false if board unavailable
	}
	for _, tile := range board.Tiles {
