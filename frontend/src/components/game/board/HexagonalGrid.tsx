...
export default function HexagonalGrid({ gameState, onHexClick }: HexagonalGridProps) {
  ...
  const getTileData = (hexCoordinate: string) => {
    ...
    if (gameState?.board?.tiles) {
      const tile = gameState.board.tiles.get?.(hexCoordinate) || gameState.board.tiles[hexCoordinate];
      if (tile) {
        return {
    ...
    <>
      {hexGrid.map((hex, index) => {
        ...
        return (
          <HexTile
            key={hexKey}
            position={[hex.cartesianPosition.x, hex.cartesianPosition.y, hex.cartesianPosition.z]}
