...
const DemoSetupOverlay: React.FC<DemoSetupOverlayProps> = ({ game, playerId }) => {
  ...
  const handleProductionChange = (resource: keyof ProductionDto, value: number) => {
    ...
    setProduction((prev) => ({ ...prev, [resource]: Math.max(minValue, value) }));
  };
      ...
      <div className={`${OVERLAY_CONTAINER_CLASS} max-w-[1000px]`}>
        ...
        <div className={OVERLAY_HEADER_CLASS}>
          ...
          <p className={OVERLAY_DESCRIPTION_CLASS}>
            Configure your starting setup. {isHost ? "As host, you can also set global parameters." : ""}
          </p>
        </div>
                  ...
                  {resourceTypes.map(({ key, icon, label }) => (
                    <div key={key} className="flex items-center gap-2">
                      ...
                      <input
                        ...
                        value={resources[key]}
                        onChange={(e) =>
                          handleResourceChange(key, parseInt(e.target.value) || 0)
                        }
                  ...
                  {resourceTypes.map(({ key, icon, label }) => (
                    <div key={key} className="flex items-center gap-2">
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          handleProductionChange(key, parseInt(e.target.value) || 0)
                        }
                        className="w-16 bg-black/60 border border-space-blue-400/50 rounded py-1 px-2 text-white text-sm text-center outline-none focus:border-space-blue-400"
