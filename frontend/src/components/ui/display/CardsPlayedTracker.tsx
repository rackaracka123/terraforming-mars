...
const CardsPlayedTracker: React.FC<CardsPlayedTrackerProps> = ({ 
  ...
  // Count cards by type
  const cardCounts = {
    [CardType.AUTOMATED]: playedCards.filter(card => card.type === CardType.AUTOMATED).length,
    [CardType.ACTIVE]: playedCards.filter(card => card.type === CardType.ACTIVE).length,
    [CardType.EVENT]: playedCards.filter(card => card.type === CardType.EVENT).length,
    [CardType.CORPORATION]: playedCards.filter(card => card.type === CardType.CORPORATION).length,
    [CardType.PRELUDE]: playedCards.filter(card => card.type === CardType.PRELUDE).length,
  };

  const cardTypeInfo = [
    { type: CardType.AUTOMATED, icon: '/assets/misc/corpCard.png', label: 'AUTO' },
    { type: CardType.ACTIVE, icon: '/assets/misc/corpCard.png', label: 'ACT' },
    { type: CardType.EVENT, icon: '/assets/tags/event.png', label: 'EVT' },
    { type: CardType.CORPORATION, icon: '/assets/misc/corpCard.png', label: 'CORP' },
  ...
  return (
    <button
      ...
      style={{
        ...
        alignItems: 'center',
        gap: dimensions.gap,
        background: 'linear-gradient(135deg, rgba(40, 60, 80, 0.9) 0%, rgba(30, 50, 70, 0.8) 100%)',
      ...
      onMouseEnter={(e) => {
        if (onClick) {
          e.currentTarget.style.background = 'linear-gradient(135deg, rgba(50, 70, 90, 0.95) 0%, rgba(40, 60, 80, 0.9) 100%)';
      ...
      onMouseLeave={(e) => {
        if (onClick) {
          e.currentTarget.style.background = 'linear-gradient(135deg, rgba(40, 60, 80, 0.9) 0%, rgba(30, 50, 70, 0.8) 100%)';
