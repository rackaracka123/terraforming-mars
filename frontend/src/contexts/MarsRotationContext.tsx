...
export function MarsRotationProvider({ children }: MarsRotationProviderProps) {
  ...
  return (
    <MarsRotationContext.Provider value={{ marsGroupRef }}>{children}</MarsRotationContext.Provider>
  );
}
...
export function useMarsRotation() {
  ...
  if (!context) {
    throw new Error("useMarsRotation must be used within a MarsRotationProvider");
  }
  return context;
}
